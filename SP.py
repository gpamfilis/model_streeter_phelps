...
def graphtwolines(x1, y1, x2, y2):
    ...
    plt.show()
def RMSE(cf, cm): #root mean square error
    n = len(cf)
    differences = []
    squared_difference = []
...
def streeter_phelps(Csat, C0, L0, ka, kd, time): #time is a 1-D array or list. #Streeter-Phelps
    C = lambda t: Csat - ((Csat - C0)*np.exp(-ka*t))-((kd*L0)/(ka-kd))*(np.exp(-kd*t)-np.exp(-ka*t)) #this is the streeter-phelps equation
...
numberoftrials = 1000
kas = np.linspace(0., 1, num=numberoftrials)
RMSES = np.zeros(numberoftrials)
for i in range(numberoftrials):
    RMSES[i] = RMSE(DO, streeter_phelps(Csat, C0, L0, kas[i], kd, time))
#find the the minimum error in the RMSES array. find the index (location) of that value. apply that location to kas list to find the
#optimum ka with the smalest error.
kamin1 = kas[np.where(RMSES == np.min(RMSES))] #minimum ka.
print "the minimum ka is: ", kamin1,"and the corresponding RMSE is: ",np.min(RMSES)
...
for i in range(values):
    kas2[i] = ranf()
    kds2[i] = ranf()
    RMSE2[i] = RMSE(DO,streeter_phelps(Csat, C0, L0, kas2[i], kds2[i], time))
...
def triangle3dplot():
    ...
    ax.set_title("RMSE versus ka and kd")
...
print "The smallest RMSE was: ",np.min(RMSE2)," for ka equal to: ",kamin," and kd equal to: ",kdmin
...
def graphtwolines(x1,y1,x2,y2):
    ...
    plt.legend(loc = "upper right")
    plt.show()
#%load C:\Users\giorgos\TUC\ΕΠΜ\Εργαστήρια\Εργαστήριο 1\lab1modeling\Εργαστήριο-1-ΕΠΜ.py
